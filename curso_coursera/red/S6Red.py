...
def opcion_menu():
    ...
    print("  5. Agrega nuevo amigo") #NUEVA OPCIÓN AÑADIDA
    print("  6. Ver últimos mensajes de mis amigos") #NUEVA OPCIÓN AÑADIDA
    print("  0. Salir")
...
def agregar_amigo(amigos): #nueva función que agrega un amigo a la lista
	nombre = input("Dime como se llama tu nuevo amigo. ")
	amigos.extend([nombre])
	return amigos

def ultimos_estados(nombre,amigos):
	#1) Abrimos como lectura nuestro fichero
	archivo_usuario = open(nombre+".user","r")
	#2) Recorremos y metemos en una lista todos los mensajes de amigos
	mensajeAmigos=list()
	nuevalinea=" "
	while nuevalinea!="":
		nuevalinea = archivo_usuario.readline().rstrip()
		if ":" in nuevalinea:
			mensajeAmigos.append(nuevalinea)
	#3) Para ver los últimos, damos la vuelta a la lista
	mensajeAmigos = mensajeAmigos[::-1]
	#4) Recorremos la lista de los amigos
	resultado=[]
	for cada in amigos: # Por cada amigo
		for mensaje in mensajeAmigos:
			if cada in mensaje:
				resultado.append(mensaje)
				break
	#FIN) cierra el archivo y devuelve los mensajes
	archivo_usuario.close()
	return resultado
