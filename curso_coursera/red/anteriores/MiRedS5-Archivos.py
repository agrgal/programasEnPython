...
if Red.checkArchivo(nombre):
	(nombre,edad,estatura,estatura_m,estatura_cm,sexo,pais,num_amigos,estado)=Red.leerArchivo(nombre)
else:
    print()
...
if os.path.isfile(nombre.upper()+".user"): # en mayúsculas, permite que siemrpe se escriba igual 
    #Esto lo hacemos si ya había un usuario con ese nombre
    print("Leyendo datos de usuario", nombre, "desde archivo.")
    archivo_usuario = open(nombre.upper()+".user","r")
...
while opcion != 0:
	...
	if opcion == 1:
		...
		Red.grabarArchivo(nombre,edad,estatura_m,estatura_cm,sexo,pais,num_amigos,estado)
		'''
