...
def main():
	...
	# 10.- Sonido
	explosion = pygame.mixer.Sound("sonido/explosion.wav")
	...
	while True:
		...
		for eventos in pygame.event.get():
			...
			if (keys[K_SPACE] and estado == 0): # comprobar estado evita 
				...
				explosion.play(0) # Suena la explosión
		...
		texto = "Marcador: %d alcances" % (marcador)
		mensaje = fuente.render(texto, 3, blanco)
		screen.blit(mensaje,(50,20)) # coloco la diana
		
		# Para disparar la bala	
		if estado == 1:
			alcanzado = bala.actualizar(time, canon.angle, diana) # se actualiza la bala
			# ==============================================================
			# Ampliación 2: Marcador, una vez definida la fuente
			# ==============================================================			
			if alcanzado == 1:
				marcador += 1 # aumenta 1 el marcador
			# ==============================================================
			# Ampliación 1: Que la bala parezca que sale de la boca del cañón
			# ==============================================================
			# calculo un valor que más o menos es a partir de cuando se ve 
			# la trayectoria de la bala del cañón
			tope = ALTO-15 - math.sin(canon.angle*PI/180)*(canon.rect.height/(3*math.cos(canon.angle*PI/180)+1))
			# fórmula experimental
			if bala.tiempo>VINI*math.sin(canon.angle*PI/180)/9.8:
				# está bajando y
				tope = ALTO;
			# ==============================================================			
			if (bala.coordy>=0 and bala.rect.centerx<=ANCHO and alcanzado==0):
				...
				print bala.rect.centery, tope
				if bala.rect.centery<=tope: 					
		...
		# Puesto aquí el dibujo del cañón "tapa" a la bala
